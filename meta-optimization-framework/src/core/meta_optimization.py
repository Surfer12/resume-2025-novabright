...
class CognitiveSymbolicReasoner(SymbolicReasoner):
    ...
    def _n_back_rules(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(n, seq_len):
            similarity = torch.cosine_similarity(
                input_data[:, i, :], input_data[:, i - n, :], dim=1
            )
    ...
    def _apply_goal_hierarchy(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        return (
            torch.cumsum(input_data, dim=1)
            / torch.arange(1, input_data.size(1) + 1).float()
        )
    ...
    def _apply_pattern_matching(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        if len(input_data.shape) == 3:
            return torch.conv1d(
                input_data.transpose(1, 2), kernel, padding=1
            ).transpose(1, 2)
        return input_data
...
class CognitiveNeuralProcessor(NeuralProcessor):
    ...
    def _build_working_memory_network(self, input_size: int) -> nn.Module:
        ...
        return nn.LSTM(
            input_size, hidden_size=input_size, num_layers=2, batch_first=True
        )
    ...
    def _build_default_network(self, input_size: int) -> nn.Module:
        ...
        return nn.Sequential(
            nn.Linear(input_size, 128), nn.ReLU(), nn.Linear(128, input_size)
        )
...
class MetaOptimizer:
    ...
    def meta_optimize(
        ...
        """
        logger.info(
            f"Starting meta-optimization for task: {task_specification.task_type}"
        )
        ...
        for iteration in range(max_iterations):
            try:
                ...
                symbolic_output = symbolic_component.process(
                    task_specification.input_data
                )
                ...
                task_loss = self._compute_task_loss(
                    hybrid_output, task_specification.target_output
                )
                cognitive_penalty = self.cognitive_regularizer.compute_penalty(
                    hybrid_output
                )
                ...
                total_loss = (
                    task_loss
                    + lambda_1 * cognitive_penalty
                    + lambda_2 * efficiency_penalty
                )
                ...
                regularization_term = (
                    torch.tensor(lambda_1) * cognitive_penalty
                    + torch.tensor(lambda_2) * efficiency_penalty
                )
                psi_x = self._compute_cognitive_computational_state(
                    hybrid_output, torch.exp(-regularization_term), bias_adjusted_output
                )

                # STEP 5: Evaluate performance
                current_performance = self._evaluate_performance(
                    psi_x, task_specification
                )

                # STEP 6: Adaptive parameter updates
                alpha = self._update_alpha(
                    alpha, symbolic_output, neural_output, current_performance
                )
                ...
                beta = self._update_beta(
                    beta, bias_adjusted_output, current_performance
                )
                ...
                if iteration % 100 == 0:
                    logger.info(
                        f"Iteration {iteration}: Performance = {current_performance:.4f}, "
        ...
        logger.info(
            f"Optimization completed. Final performance: {final_performance:.4f}"
        )
    ...
    def _compute_efficiency_penalty(
        self, symbolic_component: SymbolicReasoner, neural_component: NeuralProcessor
    ) -> float:
    ...
    def _compute_cognitive_computational_state(
        self,
        hybrid_output: torch.Tensor,
        regularization_term: torch.Tensor,
        bias_adjusted_output: torch.Tensor,
    ) -> torch.Tensor:
    ...
    def _evaluate_performance(
        self, psi_x: torch.Tensor, task_spec: TaskSpecification
    ) -> float:
        ...
        targets = (
            torch.argmax(task_spec.target_output, dim=-1)
            if task_spec.target_output.dim() > 1
            else task_spec.target_output
        )
    ...
    def _update_beta(
        self, beta: float, bias_adjusted_output: torch.Tensor, performance: float
    ) -> float:
    ...
    def _compute_confidence_interval(
        ...
        return ConfidenceInterval(
            lower=mean_perf - margin, upper=mean_perf + margin, confidence=0.95
        )
...
def main():
    ...
    parser.add_argument(
        "--task",
        choices=["n_back", "stroop", "planning", "pattern_recognition"],
        default="n_back",
    ...
    print(
        f"Confidence Interval: [{result.confidence_interval.lower:.3f}, {result.confidence_interval.upper:.3f}]"
    )
    print(
        f"Effect Size: {result.effect_size.cohens_d:.3f} ({result.effect_size.interpretation})"
    )
    print(
        f"Final Parameters: α={result.alpha:.3f}, λ₁={result.lambda_1:.3f}, λ₂={result.lambda_2:.3f}, β={result.beta:.3f}"
    )
