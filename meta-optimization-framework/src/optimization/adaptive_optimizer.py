...
from ..core.meta_optimization import OptimizationResult, TaskSpecification
...
class AdaptiveOptimizer:
    ...
    def optimize(
        ...
        for iteration in range(max_iterations):
            ...
            if iteration % 100 == 0:
                self.logger.info(
                    f"Iteration {iteration}: Loss = {step_result['loss']:.6f}, "
    ...
    def _analyze_problem(self, task_specification: TaskSpecification) -> None:
        ...
        if hasattr(data, "shape"):
            characteristics["dimensionality"] = (
                np.prod(data.shape) if len(data.shape) > 1 else len(data)
            )
        ...
        if hasattr(data, "std"):
            characteristics["noise_level"] = (
                float(data.std() / data.mean()) if data.mean() != 0 else 0.0
            )
    ...
    def _gradient_based_update(
        self, current_loss: float, gradient_norm: float
    ) -> Dict[str, Any]:
        ...
        if len(self.adaptation_state.performance_history) > 1:
            ...
            if loss_change > 0:  # Loss increased
                ...
                self.adaptive_params[
                    "learning_rate"
                ] *= 1.05  # Increase learning rate slightly

        # Adapt momentum based on gradient norm stability
        if gradient_norm > 10.0:  # High gradient norm
            self.adaptive_params["momentum"] = min(
                0.99, self.adaptive_params["momentum"] * 1.02
            )
        elif gradient_norm < 0.1:  # Low gradient norm
            self.adaptive_params["momentum"] = max(
                0.5, self.adaptive_params["momentum"] * 0.98
            )
    ...
    def _bayesian_update(self, current_loss: float) -> Dict[str, Any]:
        ...
        return {
            "method": "bayesian",
            "performance_trend": (
                performance_trend if "performance_trend" in locals() else 0.0
            ),
        }
    ...
    def _default_update(self, current_loss: float) -> Dict[str, Any]:
        ...
        return {"method": "default", "adjustment": "learning_rate_only"}
    ...
    def _check_convergence(self, threshold: float) -> bool:
        ...
        if len(self.adaptation_state.performance_history) > 2 * self.patience:
            long_recent = self.adaptation_state.performance_history[
                -2 * self.patience : -self.patience
            ]
    ...
    def get_adaptation_summary(self) -> Dict[str, Any]:
        ...
        if self.adaptation_state.adaptation_history:
            for param_name in self.adaptive_params.keys():
                values = [
                    params[param_name]
                    for params in self.adaptation_state.adaptation_history
                ]
